...
$objPHPExcel->setActiveSheetIndex($pestana)
			->setCellValue('D1','CONSTANCIA DE PRE-MATRÍCULA')
			->setCellValue('C3','CÓDIGO:')
			->setCellValue('C4','APELLIDOS Y NOMBRES:')
			->setCellValue('C5','LOCAL ESTUDIOS:')
			->setCellValue('C6','CARRERA PROFESIONAL:')
...
$objPHPExcel->getActiveSheet()->getStyle('F6:I6')->applyFromArray($styleBold);
...
$objPHPExcel->getActiveSheet()->getStyle('C6:E6')->applyFromArray($styleRigth);
...
$objPHPExcel->getActiveSheet()->setCellValue('B10','CODIG');
$objPHPExcel->getActiveSheet()->setCellValue('C10','ASIGNATURAS');
...
$objPHPExcel->getActiveSheet()->getStyle('B11:D'.($posfin-1))->applyFromArray($styleLeft);
...
/**********************************************************************/
$objPHPExcel->getActiveSheet()->setCellValue('A7','FECHA DE MATRÍCULA:'.$rs['fecha_matric'].' / FECHA DE INICIO DE CLASES:'.$rs['finicio']);
...
$pospension++;
$objPHPExcel->getActiveSheet()->setCellValue('G'.$pospension,'Inscripción');
$objPHPExcel->getActiveSheet()->mergeCells('G'.$pospension.':H'.$pospension);
$pospension++;
...
$objPHPExcel->getActiveSheet()->setCellValue('I'.($pospension-3),$fecha_inscripcion);
...
$objPHPExcel->getActiveSheet()->setCellValue('J'.($pospension-3),$monto_inscripcion);
$objPHPExcel->getActiveSheet()->setCellValue('J'.$pospension,'=SUM(J'.($pospension-3).':J'.($pospension-1).')');
$objPHPExcel->getActiveSheet()->getStyle('G'.($pospension-5).':J'.$pospension)->applyFromArray($styleThinBlackBorderAllborders);

$objPHPExcel->getActiveSheet()->getStyle('G'.($pospension-5).':J'.($pospension-4))->applyFromArray($styleAlignmentBold);
$objPHPExcel->getActiveSheet()->getStyle('G'.($pospension-5).':J'.($pospension-4))->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('DCE6F1');
...
$objPHPExcel->getActiveSheet()->setCellValue('A'.($posfin+1),'Qué debo de decir?');
...
$posfin++;//36
$posfin++;//37
$objPHPExcel->getActiveSheet()->setCellValue('A'.$posfin,'ACCEDER A LA INTRANET Y AL CAMPUS VIRTUAL');
$objPHPExcel->getActiveSheet()->mergeCells('A'.$posfin.':J'.$posfin);
$objPHPExcel->getActiveSheet()->getStyle('A'.$posfin.':J'.$posfin)->applyFromArray($styleAlignmentBold);
$objPHPExcel->getActiveSheet()->getStyle('A'.$posfin.':J'.$posfin)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('DCE6F1');

$posfin++;//38
$textourl="Para que usted pueda registrar su matrícula, la institución pone a su servicio el acceso a la Intranet a través de www.cpdtelesup.com ; para ello use el código del alumno (".$rs['dcodlib'].") como usuario y como contraseña.  En caso de presentar deuda pendiente de pago, este servicio será suspendido temporalmente hasta la cancelación de la misma.";
if($rs['durlcam']!=''){
$objPHPExcel->getActiveSheet()->getStyle('A'.$posfin)->getFont()->setSize(10);
$textourl="Para que pueda registrar su matrícula, la institución pone a su servicio el acceso a la Intranet a través de www.cpdtelesup.com cuyo usuario y contraseña es el mismo código del alumno (".$rs['dcodlib']."). Para que pueda realizar sus actividades académicas se le da el acceso al Campus Virtual a través de www.campus.telesupvirtual.com cuyo usuario es: ".$rs['dcodlib']." y su contraseña es ".$rs['dcodlib']."V.  En caso de presentar deuda pendiente de pago, este servicio será suspendido temporalmente hasta la cancelación de la misma.";
}
$objPHPExcel->getActiveSheet()->setCellValue('A'.$posfin,$textourl);
$objPHPExcel->getActiveSheet()->mergeCells('A'.$posfin.':J'.$posfin);
...
$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(7.75); // ancho   86
$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(19.9); // ancho  14
$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(18.6); // ancho   27
$objPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(7.75); // ancho  71
...
$objPHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(8.6); // ancho  71
...
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(15.5); // altura
$pos++;
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(60.5); // altura
$pos++;
...
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(15.5); // altura
$pos++;
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(36.5); // altura
$pos++;
...
$objDrawing->setName('PHPExcel');
$objDrawing->setDescription('PHPExcel');
$objDrawing->setPath('includes/firma.jpg');
$objDrawing->setHeight(60);
$objDrawing->setCoordinates('H'.$pos);
...
$pos++;
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(15.5); // altura
$objPHPExcel->getActiveSheet()->setCellValue('E'.$pos,'DIRECCIÓN ACADÉMICA');//DE LA ODE DE '.$rs['dfilial_est']
...
$pos++;
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(15.5); // altura
$objPHPExcel->getActiveSheet()->setCellValue('E'.$pos,$rs['direccion_estudio']);
$objPHPExcel->getActiveSheet()->mergeCells('E'.$pos.':J'.$pos);
$objPHPExcel->getActiveSheet()->getStyle('E'.$pos.':J'.$pos)->applyFromArray($styleBold);
$pos++;
...
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(2.75); // altura
$pos++;
$objPHPExcel->getActiveSheet()->getRowDimension($pos)->setRowHeight(6.5); // altura
$pos++;
...
header('Content-Type: application/vnd.ms-excel');
            header('Content-Disposition: attachment;filename="Pre_Matricula_ADM_'.date("Y-m-d_H-i-s").'.xls"'); // file name of excel
            header('Cache-Control: max-age=0');
